...
export default function ResetPassword() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setMessage({ text: response.data?.message || 'Contraseña restablecida con éxito.', type: 'success' });
            setTimeout(() => navigate('/login'), 3000);
        } catch (error) {
            setMessage({ text: error.response?.data?.message || 'Ocurrió un error. Inténtalo de nuevo.', type: 'error' });
        } finally {
            setLoading(false);
        }
    };
        ...
        return (
            <div className="flex flex-col items-center justify-center h-[calc(100vh-80px)] w-full px-4 overflow-hidden">
                <div className="w-full max-w-md bg-white/40 backdrop-blur-2xl shadow-[0_8px_32px_0_rgba(31,38,135,0.07)] rounded-[32px] p-6 md:p-8 border border-white/60 text-center">
                    <h2 className="mb-4 text-2xl font-extrabold text-slate-800">Enlace inválido</h2>
                    <p className="mb-6 text-sm text-slate-600 font-medium">No se encontró ningún token de recuperación en la URL.</p>
