...
export default function CheckoutModal({ isOpen, onClose, selectedAtt, onSubmitCheckout, initialStep = 'scan' }) {
  ...
  const {
    ...
    try {
      ...
      validatedTokenRef.current = parsed?.token ?? decodedText;
    } catch (e) {
        ...
        <>
          <div className="mb-2 mx-auto flex items-center justify-center gap-2" style={{ maxWidth: '300px' }}>
            ...
            >
              ...
              <span className="text-[11px]">{facingMode === 'environment' ? 'Frontal' : 'Trasera'}</span>
            </button>
          </div>

          <div
            className="relative mx-auto rounded-2xl overflow-hidden shadow-sm flex items-center justify-center border border-white/20"
            style={{ width: '100%', maxWidth: '280px', aspectRatio: '1 / 1', borderRadius: '16px', margin: '0 auto 8px', backgroundColor: '#000' }}
          >
            {!isScannerReady && (
  ...
  return (
    <GlassModal
      isOpen={isOpen}
      toggle={handleClose}
      title={selectedAtt ? selectedAtt.formation.name : t('dashboard.formationDetails')}
      size="sm"
    >
