...
function DropdownOptionItem({ opt, isSelected, onSelect }) {
  const selectedStyle = isSelected 
    ? 'bg-[#b3c34c]/20 text-[#3b4707] dark:text-[#d4e84a] font-bold' 
...
export default function GlassDropdown({ 
  ...
  const buttonSizeClass = compact 
    ? 'min-h-[32px] px-2.5 py-1 text-xs rounded-xl font-bold' 
    : 'min-h-[44px] px-4 py-2 text-xs sm:text-sm rounded-2xl font-medium';

  const buttonOpenClass = isOpen ? 'ring-2 ring-[#b3c34c]/50 border-[#b3c34c]/60 shadow-md bg-white dark:bg-slate-900' : '';
  const chevronSizeClass = compact ? 'text-[9px]' : 'text-[11px]';
  const chevronOpenClass = isOpen ? 'rotate-180 text-[#7a8a18]' : 'text-slate-400 dark:text-slate-500';
  ...
  return (
    <div ref={dropdownRef} className={`relative w-full ${isOpen ? 'z-50' : 'z-20'} ${className}`}>
      <button
        ...
        disabled={disabled}
        onClick={toggle}
        className={`da-glass-dropdown-trigger w-full flex items-center justify-between text-left transition-all duration-200 border border-white/80 dark:border-white/15 bg-white/75 dark:bg-slate-900/80 hover:bg-white/95 dark:hover:bg-slate-900/95 backdrop-blur-md shadow-xs active:scale-[0.99] disabled:opacity-50 disabled:cursor-not-allowed ${buttonSizeClass} ${buttonOpenClass}`}
      >
        <span className="truncate flex-1 text-slate-800 dark:text-slate-100">{displayLabel}</span>
