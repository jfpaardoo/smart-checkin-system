...
function AppNavbar() {
    ...
    // Estado principal para menús del nivel superior (escritorio y notificaciones móvil)
    const [openMenu, setOpenMenu] = useState(null);
    
    // Estado secundario exclusivo para desplegar subidiomas dentro del menú móvil
    ...
    if (roles.includes("ADMIN")) {
        adminLinks = (
            <div className="relative dropdown-container">
                <button type="button" onClick={(e) => toggleMenu('admin', e)} className={navBtnClass}>
                    <FaUserShield />
                ...
                <div className={`${dropdownContainerClassLeft} transition-all duration-300 origin-top-left ${isAdminOpen ? 'opacity-100 scale-100 translate-y-0 visible' : 'opacity-0 scale-95 -translate-y-4 invisible pointer-events-none'}`}>
                    <div className="py-1" role="menu">
                        <Link to="/users" className={dropdownItemClass} onClick={closeAll}><FaUsers className="text-white/60"/> {t('nav.manageUsers')}</Link>
                        <Link to="/formations" className={dropdownItemClass} onClick={closeAll}><FaGraduationCap className="text-white/60"/> {t('nav.manageFormations')}</Link>
                        <Link to="/analytics" className={dropdownItemClass} onClick={closeAll}><FaChartLine className="text-white/60"/> {t('nav.analytics')}</Link>
                        <Link to="/audit" className={dropdownItemClass} onClick={closeAll}><FaShieldAlt className="text-white/60"/> Auditoría</Link>
                        <Link to="/admin/cloud-settings" className={dropdownItemClass} onClick={closeAll}><FaCloudUploadAlt className="text-white/60"/> Ajustes de Nube</Link>
                        <div className="border-t border-white/20 my-1 mx-2"></div>
                        <Link to="/qr-generator" className={dropdownItemClass} onClick={closeAll}><FaQrcode className="text-white/60"/> {t('nav.qrGenerator')}</Link>
                        <Link to="/docs" className={dropdownItemClass} onClick={closeAll}><FaBookOpen className="text-white/60"/> {t('nav.docs')}</Link>
                    </div>
                </div>
            </div>
        );
    ...
    if (!jwt) {
        ...
        userLogout = (
            <div className="relative dropdown-container">
                <button type="button" onClick={(e) => toggleMenu('user', e)} className={navBtnClass}>
                ...
                <div className={`${dropdownContainerClassRight} transition-all duration-300 origin-top-right ${isUserOpen ? 'opacity-100 scale-100 translate-y-0 visible' : 'opacity-0 scale-95 -translate-y-4 invisible pointer-events-none'}`}>
                    <div className="py-1" role="menu">
                        <Link to="/profile" className={dropdownItemClass} onClick={closeAll}><FaIdCard className="text-white/60"/> Mi Perfil</Link>
                        <div className="border-t border-white/20 my-1 mx-2"></div>
                        <Link to="/logout" className={dropdownItemClass} onClick={closeAll}><FaSignOutAlt className="text-red-400"/> Salir</Link>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <nav className="ba-nav-capsule">
            <div className="w-full px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center h-[64px]">
                    <div className="flex items-center">
                        <Link to="/" className="flex items-center gap-4 group text-white hover:text-white no-underline" onClick={closeAll}>
            ...
            {/* Panel Desplegable Móvil - Menú Principal */}
            <div className={`md:hidden transition-all duration-500 ease-in-out w-full ${!isNavMobileOpen ? 'max-h-0 opacity-0 overflow-hidden' : 'max-h-[85vh] opacity-100 bg-black/20 border-t border-white/10 rounded-b-4xl overflow-y-auto'}`}>
                    ...
                    <div className="flex flex-col gap-2 py-2">
                        {!jwt && (
                            <div className="flex flex-col gap-2">
                                <Link to="/register" className={dropdownItemClass} onClick={closeAll}><FaUserPlus className="text-white/60"/> {t('nav.register', 'Solicitar Registro')}</Link>
                                <Link to="/login" className="flex items-center gap-3 px-[18px] py-[10px] my-1 text-[0.95rem] bg-ba-primary text-ba-dark rounded-[20px] font-semibold" onClick={closeAll}><FaSignInAlt /> {t('nav.login', 'Iniciar Sesión')}</Link>
                            </div>
                        )}
                        {jwt && (
                        ...
                        {/* Selector de idioma expansible internamente en móvil */}
                        <div className="pt-4 flex flex-col items-center border-t border-white/10 mt-2 text-white">
                            ...
                            <div className={`w-full transition-all duration-300 overflow-hidden ${mobileLangOpen ? 'max-h-60 mt-3 opacity-100' : 'max-h-0 opacity-0'}`}>
                                <div className="bg-white/5 rounded-2xl p-2 space-y-1 border border-white/10 max-h-48 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                                    {LANGUAGES.map(({ code, label }) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`ba-nav-dropdown-item w-full flex items-center justify-between text-left text-xs ${
                                                    isSelected ? 'bg-white/10 font-bold' : ''
                                                }`}
                                            >
                                                <span>{label}</span>
                                                {isSelected && <FaCheck className="text-[#b3c34c] text-xs ml-2" />}
                                            </button>
                                        );
                                    })}
                                </div>
            ...
            <div className={`md:hidden transition-all duration-500 ease-in-out w-full ${!isNotifOpen ? 'max-h-0 opacity-0 overflow-hidden' : 'max-h-[60vh] opacity-100 bg-black/20 border-t border-white/10 rounded-b-4xl overflow-hidden'}`}>
